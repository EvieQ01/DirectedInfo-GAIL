...
def main(args):
    ...
    # x  -> 1
    discriminator = Discriminator(input_size=len(expert.set_diff)).type(dtype)
    print(discriminator)

    # x  -> c
    netQ = QHead(input_size=len(expert.set_diff), output_size=args.c_dim).type(dtype)
    print(netQ)

    optimD = optim.Adam([{'params': discriminator.parameters()}], lr=args.lr)#, betas=(args.beta1, args.beta2))
    ...
    for epoch in trange(args.num_epochs):
        ...
        for iter in range(dataset.size // args.batch_size):
            # batch, mask = dataset.sample_padded_batch()
            batch, boundary_index = dataset.sample_packed_batch()
            # batch, boundary_index = dataset.sample_specific_batch(0)
            # batch = batch.type(dtype)
            batch = batch.to(device)
            # Updating discriminator and DHead
            optimD.zero_grad()
            # Real data
            label = torch.full((args.batch_size, ), real_label).type(dtype=dtype)
            probs_real = torch.sigmoid(discriminator(batch)).view(-1) # (B, max_len, dim) -> (B, 1)
            ...
            noise, idx = noise_sample(args.c_dim, z_dim=args.z_dim, batch_size=args.batch_size, dtype=dtype) # (B, c+z)
            init_state = dataset.init_state_sample()
            # fake_data = netG(x_0=init_state, z_c=noise) #TODO (B, max_len, dim)
            fake_data = netG(x_0=batch, z_c=noise, teacher_force=args.teacher_force) #TODO (B, max_len, dim) [packed]
            
            # fake_data = torch.softmax(fake_data, dim=-1)
            # detach!
            fake_data_unpacked, lens_unpacked = pad_packed_sequence(fake_data, batch_first=True)
            fake_data_detach = pack_padded_sequence(fake_data_unpacked.detach(), lengths=lens_unpacked, \
                            batch_first=True, enforce_sorted=False)
            
            probs_fake = torch.sigmoid(discriminator(fake_data_detach)).view(-1)
            # pdb.set_trace()
            ...
            probs_fake = torch.sigmoid(discriminator(fake_data)).view(-1)
            label.fill_(real_label)
            gen_loss = criterionD(probs_fake, label)

            q_logits = netQ(fake_data) # change this from origin infoGAN
            target = torch.tensor(idx).type(dtype=label_dtype)
            # Calculating loss for discrete latent code.
            #(B, c_dim), (B, c_dim)
            # pdb.set_trace()
            posterior_loss = criterionQ(q_logits, target)
            ...
            mask = mask + mask.T
            q_logits_detach = netQ(fake_data_detach) 
            ...
            dist_loss = torch.sqrt(torch.mean(torch.sum(dist*dist, dim=-1)* mask / 2 ) ) 
            # Net loss for generator.
            G_loss = gen_loss + posterior_loss * args.lambda_post + 1 / (dist_loss + 1e-5) * args.lambda_dist
            # Calculate gradients.
            # pdb.set_trace()
            G_loss.backward()
            if epoch == 100:
        ...
        if epoch % 10 == 0:
            state_and_pred_context = test_for_context(Qnet=netQ, expert=expert, boundary_list=boundary_list,\
                                                        dataset=dataset, dtype=dtype, num_test_samples=10)

            # plot
            fig_dir = os.path.join(os.path.dirname(args.results_pkl_path), 'visualize')
            os.makedirs(fig_dir,exist_ok='True')
            plot_trajectory(state_and_pred_context,
                            (15, 11),
...
def test_for_context(Qnet:nn.Module, expert, boundary_list, dataset, dtype, num_test_samples=10):
    Qnet.eval()
    ...
    for i_traj in range(num_test_samples):
        ...
        for t in range(ep_state.shape[1]):
            ...
            if t in boundary_time_stamp_list or t == 0:
                ...
                if t != 0:
                    logits_q = F.softmax(Qnet(dataset.turn_xy_posi_to_onehot(x_var[0]).type(dtype).unsqueeze(0)), dim=-1)[0] #(B, c)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_post', type=float, default=.001, help='param for gen')
